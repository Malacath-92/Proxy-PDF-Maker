...
struct Project
{
    ...
    void Load(const fs::path& json_path, const cv::Mat* vibrance_cube, PrintFn print_fn);
    void Dump(const fs::path& json_path, PrintFn print_fn) const;

    void Init(const cv::Mat* vibrance_cube, PrintFn print_fn);
    void InitProperties(PrintFn print_fn);
    void InitImages(const cv::Mat* vibrance_cube, PrintFn print_fn);
