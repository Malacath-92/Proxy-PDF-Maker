...
template<class T>
using Lazy = std::optional<T>;

struct AppContext
{
    Lazy<Log> m_MainLog;
    Lazy<PrintProxyPrepApplication> m_App;
    Project m_Project;
...
extern "C"
{
    PROXY_PDF_EXPORT void* Initialize(int argc, char** argv)
    {
        TRACY_WAIT_CONNECT();
        TRACY_AUTO_SCOPE();
        ...
        ctx->m_MainLog.emplace(log_flags, Log::c_MainLogName);

        PrintProxyPrepApplication& app{ ctx->m_App.emplace(argc, argv) };
        ...
        Cropper& cropper{ ctx->m_Cropper.emplace([](std::string_view cube_name)
                                                 { return GetCubeImage(cube_name); },
                                                 project) };
        CardProvider& card_provider{ ctx->m_CardProvider.emplace(project) };
        ...
        PluginRouter& plugin_router{ ctx->m_PluginRouter };
...

        {
            ...
            QObject::connect(card_area, &CardArea::RequestOpenPluginsWindow, global_options, &GlobalOptionsWidget::RequestOpenPluginsWindow);
